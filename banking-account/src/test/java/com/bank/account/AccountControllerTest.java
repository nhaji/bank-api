...
class AccountControllerTest extends BaseApiTest {
    ...
    void setUp() throws Exception {
        ...
        ResponseDTO<AccountDto> response = objectMapper.readValue(
    ...
    class CreateAccountTests {
        ...
        void shouldCreateAdditionalAccountForUser() throws Exception {
            MvcResult before = mockMvc.perform(get("/accounts").with(auth(PRIMARY_USER)))
            ...
            MvcResult after = mockMvc.perform(get("/accounts").with(auth(PRIMARY_USER)))
                    .andExpect(status().isOk())
                    .andReturn();
    ...
    class TransactionTests {
        ...
        void shouldNotWithdrawMoreThanBalance() throws Exception {
            MvcResult result = mockMvc.perform(post("/accounts/{accountId}/withdraw", firstAccountId)
                    ...
                    .andReturn();
            assertThat(readErrorCode(result)).isEqualTo(AccountBusinessException.INSUFFICIENT_BALANCE_CODE);
        }
    }
    ...
    class StatementTests {
        ...
        void shouldRejectStatementForUnauthorizedAccount() throws Exception {
            MvcResult otherAccount = mockMvc.perform(post("/accounts")
            ...
            ResponseDTO<AccountDto> created = objectMapper.readValue(
                    otherAccount.getResponse().getContentAsString(),
                    new TypeReference<>() {});

            MvcResult result = mockMvc.perform(get("/accounts/{accountId}/statement", created.getData().getId())
                            .with(auth(PRIMARY_USER)))
                    .andExpect(status().isBadRequest())
                    .andReturn();
            assertThat(readErrorCode(result)).isEqualTo(AccountBusinessException.ACCOUNT_ACCES_UNAUTHORIZED_CODE);
        }
    }
}
