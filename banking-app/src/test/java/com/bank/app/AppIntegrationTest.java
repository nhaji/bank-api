...
class AppIntegrationTest extends BaseIntegrationTest {
    ...
    class ManagementFlow {
        ...
        void shouldListUpdateAndDeleteUsersThroughRealApplication() throws Exception {
            ...
            MvcResult listBefore = mockMvc.perform(get("/users")
                            .header(HttpHeaders.AUTHORIZATION, bearer(token)))
                    .andExpect(status().isOk())
                    .andReturn();

            ResponseDTO<List<UserResponse>> beforeResponse = objectMapper.readValue(
                    listBefore.getResponse().getContentAsString(),
                    new TypeReference<>() {});
            ...
            MvcResult updateResult = mockMvc.perform(put("/users/{userId}", userId)
                            .header(HttpHeaders.AUTHORIZATION, bearer(token))
            ...
            ResponseDTO<UserResponse> updateResponse = objectMapper.readValue(
            ...
            MvcResult deleteResult = mockMvc.perform(delete("/users/{userId}", userId)
